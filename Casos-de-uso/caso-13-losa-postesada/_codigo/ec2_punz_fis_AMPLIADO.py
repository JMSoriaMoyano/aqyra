...
def punzonamiento(V_Ed, c1, c2, d, fck, rho_l, posicion="corner", beta=None,
    ...
    """
    # EC2 6.4.4: efecto favorable del pretensado.
    # (b) descuento de la componente vertical de los tendones que cruzan u1.
    V_Ed_bruta = V_Ed
    V_Ed = V_Ed - V_p
    d_mm = d * 1e3
    fck_MPa = fck / 1e6
    rho_l = min(rho_l, 0.02)
    sigma_cp_MPa = sigma_cp / 1e6
    ...
    C_Rd_c = 0.18 / GC
    v_Rd_c_base = max(C_Rd_c * k * (100 * rho_l * fck_MPa) ** (1 / 3),
                      0.035 * k ** 1.5 * fck_MPa ** 0.5)
    # (a) v_Rd,c += k1 * sigma_cp  (k1=0,10 [confirmar AN])
    v_Rd_c_MPa = v_Rd_c_base + k1 * sigma_cp_MPa
    ...
    return {
        "V_Ed_kN": V_Ed / 1e3, "V_Ed_red_kN": V_Ed / 1e3, "V_Ed_bruta_kN": V_Ed_bruta / 1e3,
        "V_p_kN": V_p / 1e3, "sigma_cp_MPa": sigma_cp_MPa,
        "vRdc_base_MPa": v_Rd_c_base, "k1": k1,
        "posicion": posicion, "beta": beta,
