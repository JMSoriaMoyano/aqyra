# -----------------------------------------------------------------------------
# PROCEDENCIA (PT 6.3, Ola 6 -- decision nº7 "grafo + N solvers"):
#   COPIA BYTE A BYTE del solver Darcy-Weisbach de `instalaciones`
#   (`instalaciones/scripts/red/solver_red.py`, PT 4.3/4.4) renombrada a
#   `solver_presion.py` para el vertical de ABASTECIMIENTO a presion (EN 805) de
#   `obras-lineales`. El cuerpo (clases, funciones, constantes y logica) es
#   IDENTICO al original; solo se anade este banner de procedencia. El aislamiento
#   de runtime impide importar entre plugins, por lo que el espejo/copia es la via.
#   Cualquier mejora del motor a presion debe sincronizarse con el original de
#   `instalaciones` para mantener la trazabilidad. El nucleo (grafo) NO se toca.
# -----------------------------------------------------------------------------
"""
