...
chcp 65001 >nul
setlocal
title aqyra-entorno: commit del reinicio organizativo (reorg 2026-06-29)

set "REPO=C:\Users\jmsor\Documents\Claude\Projects\Entorno"
set "URL=https://github.com/JMSoriaMoyano/aqyra-entorno.git"
set "LOG=%REPO%\_operativo\entorno_reorg_resultado.txt"

cd /d "%REPO%" || (echo No se encuentra el repo: %REPO% & pause & exit /b 1)
if not exist "%REPO%\_operativo" mkdir "%REPO%\_operativo"

echo ===== aqyra-entorno: commit del reinicio organizativo =====> "%LOG%"
...
REM --- 4) Commit ---
git commit -m "reorg: reinicio organizativo (opcion B) - HILO-* cerrados a _historial/, .bat+logs a _operativo/ (.gitignore); raiz de docs vivos. Sustrato del cebo intacto (golden y firmas sin tocar)." >> "%LOG%" 2>&1
set "RC=%errorlevel%"
