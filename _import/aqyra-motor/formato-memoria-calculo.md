...
- **Modelo:** grafo de red (parser `iso19650-openbim` + versión), solver
  (Darcy-Weisbach / Hardy-Cross en mallas / método de las intensidades).
- **Comprobaciones:** por tramo — velocidad, pérdida de carga, presión/caudal en el
  punto más desfavorable; caída de tensión (3 % alumbrado / 5 % fuerza) e intensidad
  admisible. **Write-back de Psets de resultado al IFC.**

### 4.3 Obras lineales (carreteras)
- **Normativa:** Norma 3.1-IC (trazado), 6.1-IC (firmes), 5.2-IC (drenaje); EN 752
  (saneamiento), EN 805 (abastecimiento).
- **Datos/demanda:** velocidad de proyecto V_p; categoría de tráfico (IMD, % pesados) y
  de explanada; hidrología (método racional 5.2-IC, T de retorno).
- **Modelo:** modelo neutro lineal por PK / grafo de red (parser + versión); solvers
  (Manning lámina libre, Darcy-Weisbach a presión).
- **Comprobaciones:** trazado CUMPLE/NO CUMPLE vs V_p; sección de firme del catálogo;
  capacidad de cunetas/ODT; calado/velocidad/llenado (saneamiento); velocidad/presión
  (abastecimiento). Relleno de ganchos del modelo y write-back al IFC.

### 4.4 CTE / cumplimiento normativo
- Estructura por **exigencias básicas** (DB-SI, DB-SUA, DB-HS, DB-HR, DB-HE, DB-SE) y
  normativa concurrente (RITE, REBT, RSCIEI, RIPCI). Para esto, la skill
  `cte-documentos-basicos:memoria-cte` ya define el desglose; este formato aporta la
  identidad visual (portada, estilos, descargo).

---

## 5. Cómo se aplica

- Las skills de memoria **redactan en Word** siguiendo el §3 (esqueleto) + el §4 de su
  disciplina, con la identidad del §2, partiendo de la **plantilla maestra**
  `Plantilla_Memoria_Calculo.docx`.
- Las memorias `.md` existentes (puentes, obras lineales) mantienen el mismo esqueleto y
  descargo; al pasarlas a Word se usa esta plantilla.
- Si un dato falta, se marca `[PENDIENTE]`; si depende del AN, `[confirmar AN]`. No se
