...
# nucleo transversal (PT 4.1, Ola 4): utilidades IFC + grafo de red compartidos
_NUCLEO = os.path.join(os.path.dirname(_HERE), "nucleo")
if _NUCLEO not in sys.path:
    sys.path.insert(0, _NUCLEO)
import ifc_utils   # noqa: E402  (psets, length_scale, pset_value, algebra 4x4)
import grafo_red   # noqa: E402  (RegistroNodos, troceo T/X, union-find)
...
def _length_scale(ifc):
    """Factor de unidad de longitud del IFC -> metros. Delegado al nucleo
    (ifc_utils.length_scale); se mantiene el nombre local como adaptador fino."""
    return ifc_utils.length_scale(ifc)
...
def _snap_tol_from_ifc(ifc, defecto=TOL):
    ...
    Delegado al lector puntual del nucleo (ifc_utils.pset_value)."""
    v = ifc_utils.pset_value(ifc, "Pset_Estructurando_Puente", "Snap_tol_m", None)
...
# --- algebra de matrices 4x4 -> nucleo (ifc_utils); adaptadores finos --------
_matmul = ifc_utils.matmul
_apply = ifc_utils.apply
_to_list4 = ifc_utils.to_list4
_ident4 = ifc_utils.ident4


# --- Psets -> nucleo (ifc_utils.psets); adaptador fino ----------------------
...
# --- Grafo de nudos -> nucleo (grafo_red); adaptadores finos ----------------
# La construccion de nudos+tramos (fusion por tolerancia, troceo T/X, union-find)
# vive ahora en el nucleo transversal; aqui solo se referencian las primitivas
# para mantener el comportamiento byte a byte de la serie R.
_Nodos = grafo_red.RegistroNodos


_proyeccion = grafo_red.proyeccion
_punto_en_segmento = grafo_red.punto_en_segmento
...
def _filtrar_desconectadas(model):
    ...
    El union-find generico vive en el nucleo (grafo_red); aqui el predicado de
    ancla es "nudo con algun apoyo" y se aplica la limpieza del payload
    estructural (barras, nudos huerfanos y cargas)."""
    barras = model["barras"]
    nodos = model["nodos"]
    if not barras:
        return
    bar_drop, _drop_nodos = grafo_red.filtrar_componentes_desconectadas(
        nodos, barras, lambda nm, nd: any(nd.get("apoyo", [])))
    if not bar_drop:
        return
    nodos_usados = set()
