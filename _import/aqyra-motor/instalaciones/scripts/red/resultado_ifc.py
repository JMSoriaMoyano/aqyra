"""
SEMANTICA DE LOS PSETS DE RESULTADO DE RED (write-back). Disciplina `instalaciones`,
PT 4.4 (Ola 4). Frontera C1 (decision PT 4.4, opcion a): QUE escribir lo decide la
DISCIPLINA (aqui); la MECANICA IFC (abrir/localizar/escribir/guardar) la aporta la
capa transversal `iso19650-openbim` (skill ifc-create:escribir_psets_resultado.py).

A partir del MODELO NEUTRO DE RED (con la clave `elemento` = Name del IfcFlowSegment
por tramo) y del RESULTADO del solver, construye el MAPPING JSON
...
Uso (CLI):  python3 resultado_ifc.py modelo_neutro.json resultado.json mapping.json
...
def construir_mapping(modelo, resultado):
    ...
    return {"elementos": elementos,
            "_meta": {"pset": _PSET,
                      "fuente": "instalaciones/red (solver hidraulico de red)",
