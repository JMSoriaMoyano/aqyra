#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""build_plugin_motor_fem.py — empaqueta el .plugin `motor-fem` 0.3.0 desde el monorepo.

PR-E6 (Fase I · hilo 6): cuarto y ULTIMO plugin re-homed que NACE sin espejo — con el,
Fase I completa el vaciado de espejos. `plugins/motor-fem`
...
Procedencia (ver packages/core/NUCLEO_PR-E6_decision.md): la fuente del re-home fue el
FROZEN `_import/aqyra-motor/motor-fem-v0.3.0.plugin` (23 ficheros = 0.2.1 + FEM-2), NO el
árbol `_import/aqyra-motor/motor-fem/` (0.2.1, 19 ficheros, obsoleto salvo refactor
cosmético sin cambio de comportamiento). Regla de PR-E5 aplicada: árbol solo si árbol ≥
frozen.
...
{test_grafo_red.py,README.md}` (artefactos de desarrollo del núcleo, igual que los cuatro
re-homes anteriores). En la puerta, la ausencia de un .py de la ref es AVISO,
no bloqueo: único aviso esperado = test_grafo_red.py; 0 encogidos.

Uso:
    python3 tools/build_plugin_motor_fem.py            # build + verificación intrínseca
    python3 tools/build_plugin_motor_fem.py --ref <v0.6.0.plugin>   # + contraste de tamaños

Exit 0 = APTO (dist/plugins/motor-fem-<v>.plugin listo para firmar/publicar).
"""
...
def _zip(version: str) -> Path:
    ...
    out = DIST / f"motor-fem-{version}.plugin"
...
def main() -> int:
    ...
    ap.add_argument(
        "--ref",
        default=str(ROOT / "_import" / "aqyra-motor" / "motor-fem-v0.3.0.plugin"),
        help=".plugin previo (v0.3.0, frozen — la propia fuente del re-home) para el contraste anti-truncado",
    )
    ap.add_argument(
        "--allow-shrink",
        default="",
    ...
    version = pj["version"]
    print(f"== build motor-fem {version} ==")
