# -*- coding: utf-8 -*-
"""Formato del despacho para el Documento de Presupuesto — IN-REPO, hermético (solo python-docx).

Réplica de las reglas del skill `memoria-calculo-despacho` (scripts/docx_despacho.py, la
ESPECIFICACIÓN de referencia). Se implementa aquí para que el gate de CI sea hermético (sin el
skill en el runner):

- A4, márgenes 25 mm; Arial 11 (cuerpo) y Arial 9,5 (tablas).
- Párrafos justificados; títulos a la izquierda, en azul.
- Tablas SIN cebra, cabecera azul claro (#D9E2F3) que SE REPITE en saltos de página,
  sin líneas verticales salvo en la cabecera.
- Carátula con el TOTAL (PEC) destacado en verde.
...
# ============================ documento base ============================
def nuevo_doc():
    """Documento con estilos y página A4 del despacho. Devuelve (doc, sec)."""
    doc = Document()
    n = doc.styles["Normal"]
...
def total_destacado(doc, etiqueta, valor_texto):
    """El TOTAL (PEC) destacado en verde en la carátula."""
...
def caratula(doc, titulo, subtitulo, campos, total_etq=None, total_val=None):
    for _ in range(2):
        doc.add_paragraph()
    ...
    rr = dp.add_run(
        "Documento de PRESUPUESTO generado de forma DETERMINISTA a partir de la salida "
        "autoritativa de C5 (medición trazable + cuadros de precios). Debe ser revisado y "
        "firmado por técnico competente antes de su uso.")
...
def cabecera_pie(sec, proyecto, autor):
    ...
    r = fp.add_run(autor + " . Presupuesto\t"); r.font.size = Pt(8.5); r.font.color.rgb = GRIS
