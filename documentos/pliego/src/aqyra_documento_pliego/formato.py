# -*- coding: utf-8 -*-
"""Formato del despacho para el Pliego de Condiciones Tecnicas — IN-REPO, hermetico (solo python-docx).

ESPEJO (copia in-repo, D1 opcion A) del formato de `documentos/presupuesto/formato.py`, que a su vez
replica las reglas del skill `memoria-calculo-despacho`. Se copia —no se importa— para mantener el gate
hermetico (solo `python-docx`), desacoplar versiones y evitar una dependencia
`documento-pliego -> documento-presupuesto` (paquete firmado). Si aparece un tercer documento, extraer a
un modulo comun.
