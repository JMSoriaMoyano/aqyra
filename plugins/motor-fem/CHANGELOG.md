...
## v0.3.0 — 2026-06-23 · PT 7.4 (Ola 7) · **FEM-2** (lámina curva + rigidizador + pared delgada)

Tercer peldaño del motor. **Capa ADITIVA**: `fem_core.py`, `elementos/barra.py`, `elementos/lamina.py`
(DKMQ) y `fem1.py` **intactos** (byte a byte) → la no-regresión de FEM-0/1 es **por construcción**.
C5 §8 (añadir claves/elementos, no cambiar semántica). Habilita el **cajón postesado** (puentes) y,
como spin-off, las **cubiertas laminares** de la Ola 3.

### Añadido
- **`elementos/lamina_curva.py` — `LaminaCurvaMITC4`**: cuadrilátero isoparamétrico de 4 nudos
  (24 GdL) de Reissner-Mindlin con **cortante transversal asumido MITC4** (Dvorkin-Bathe), membrana
  + **drilling** (1/1000 de la diagonal de flexión, regla de Bathe). Terna local por elemento →
  malla superficies **curvas/alabeadas**. Misma interfaz pública que `LaminaQuad` (`T/K/fer_presion/
  moment/shear/membrane`).
- **`elementos/rigidizador.py` — `ElementoRigidizador`**: barra **excéntrica** acoplada por **offset
  rígido** (`u_punto = u_nudo + θ×r`), subclase de `ElementoBarra` (no añade nudos). Diafragmas,
  almas rigidizadas, conexión losa-alma.
- **`fem2.py`**: `ElementoLaminaCurva` (subclase de `ElementoLamina` con la lámina MITC4) +
  re-export del rigidizador + **pared delgada**: `bredt_J` (torsión de celda cerrada), `shear_lag_beff`
  (ancho eficaz tipo EC2), `indicador_distorsion`. `isinstance` con las clases base se conserva →
  ensamblaje/modal/móvil del núcleo sin cambios.
- **`validacion/nafems2.py`**: Scordelis-Lo roof (1.6%), pinched cylinder (2.8%), hemispherical
  shell (0.4%), placa rigidizada vs Euler compuesta (1.3%), cajón vs Bredt (3.4%). Tolerancia **±5%**.

### Validación / no-regresión
- **FEM-2** dentro de ±5% (NAFEMS) + offset rígido 1.3% vs viga compuesta. Lámina plana
  (almas/alas del cajón) cuasi-exacta (placa 0.13%, membrana 5.6e-16); la doble curvatura
  (hemisferio) converge por refinado de malla (faceta plana).
- **FEM-0/1 EXACTO**: `test_fem_core` OK, `analitico`/`nafems`(FEM-0) OK, `fem1` (modal 4e-7, IL,
  `il_lamina` 8.6e-15) OK, `no_regresion` **SIN REGRESIÓN**.
