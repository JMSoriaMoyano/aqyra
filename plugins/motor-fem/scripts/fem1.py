...
cargas).
 ...
 - MOVIL: LINEAS DE INFLUENCIA por barrido de carga unidad sobre un camino
   ...
   ya en magnitudes (IAP-11 lo arma la disciplina `puentes`).
...
def movil(M, cfg):
    ...
    cfg = {
      "posiciones": 41,
      "objetivos": [ {"id","tipo":"esfuerzo_barra|reaccion|desplazamiento",
                      "elem","comp"} | {"nodo","comp"} ],
    ...
    for obj in objetivos:
        ...
        for ln in lineas:
            ...
            if len(s_arr) < 2:
                continue
            # IL como signo: para -Z (gravedad) la carga es negativa; eta es por
            # carga unidad +(-Z)?  _carga aplica P en -? No: dir_dof=2, P=+1 ->
            # fuerza +Z. La carga real de trafico es hacia -Z, asi que el efecto
            # = -eta * |P|. Trabajamos con eta_grav = -eta (efecto por carga
            # gravitatoria unidad).
            eta_g = -eta
            ...
            # combinacion: max y min de esta linea
