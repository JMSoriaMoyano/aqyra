"""Mallador: modelo neutro estructural (C1) -> malla FEM (C5).

Dos vias:
 - `desde_modelo_neutro(model)`: API de produccion. Barras 1D -> elementos barra;
   `superficies` -> malla estructurada de laminas (cuadrilateros). Nodos
   compartidos por tolerancia.
 - `desde_pynite(m)`: ADAPTADOR ESPEJO para el strangler. Toma un `FEModel3D`
   ...
   nucleo propio, para la NO-REGRESION (mismo modelo, dos motores).
...
def desde_pynite(m):
    ...
    solvers de produccion: distribuidas globales FX/FY/FZ uniformes y presiones
    de quad. Lanza si encuentra una carga no contemplada (para no fallar en
    silencio)."""
    M = ModeloFEM()
    ...
    for name, q in m.quads.items():
        ...
        el = ElementoLamina(name, [q.i_node.name, q.j_node.name, q.m_node.name, q.n_node.name],
                            [(q.i_node.X, q.i_node.Y, q.i_node.Z),
                             ...
                             (q.n_node.X, q.n_node.Y, q.n_node.Z)], q.t, E, nu)
...
def desde_modelo_neutro(model, estabilizar_plano=True):
    ...
    Convencion: X,Y horizontales, Z vertical; cargas `direccion:"GZ"` -> global
    uniforme en Z. Si el modelo es plano (todos los nodos comparten una
    coordenada) se coaccionan los GdL fuera de plano para estabilizar el modelo
    plano dentro del solver 3D (como el solver de barras de produccion)."""
    M = ModeloFEM()
    mats = model["materiales"]; secs = model.get("secciones", {})
    nodos = model["nodos"]
    ...
    # estabilizacion de modelo plano
...
def _releases(tipo):
    if tipo in ("PIN_JOINED_MEMBER", "TRUSS", "articulado"):
        # liberar flexion en ambos extremos (rotulas): My,Mz en i y j
        r = [False]*12
        for i in (4, 5, 10, 11):
            r[i] = True
        return r
...
def _estabilizar(M, nodos):
    ...
    out = None
    if len(ys) == 1:      # plano XZ -> fuera de plano: DY, RX, RZ
        out = [1, 3, 5]
    elif len(zs) == 1:    # plano XY -> fuera de plano: DZ, RX, RY (placa)
        out = [2, 3, 4]
    elif len(xs) == 1:    # plano YZ -> fuera de plano: DX, RY, RZ
        out = [0, 4, 5]
    if out is None:
        return
