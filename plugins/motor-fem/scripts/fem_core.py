...
class ElementoBarra:
    ...
    def fer_local_unc(self, caso):
        ...
        for c in self.cargas:
            if c["caso"] != caso: continue
            tp = c["tipo"]
            if tp == "uniforme":
                fer += _barra.fer_dist(c["q"], c["q"], 0.0, L, L, c.get("direccion", "Fy"))
            elif tp == "lineal":
                fer += _barra.fer_dist(c["q1"], c["q2"], c.get("x1", 0.0), c.get("x2", L), L, c.get("direccion", "Fy"))
                ...
                fer += _barra.fer_termica_gradiente(self.mat["E"], self.sec["Iz"], c["alpha"], c["dT"], c["h"], L)
        return fer
...
class ElementoLamina:
    def __init__(self, eid, nodos, coords, t, E, nu):
        ...
        self.Kglobal = self.quad.K(); self.cargas = []

    def dofs(self, nindex):
        out = []
        for n in self.nodos:
            b = nindex[n]*GDL; out += list(range(b, b+GDL))
        return out

    def fer_global(self, caso):
        fer = np.zeros((24, 1))
        for c in self.cargas:
            if c["caso"] == caso: fer = fer + self.quad.fer_presion(c["p"])
        return fer

    def esfuerzos(self, U, nindex, caso=None):
        d = U[self.dofs(nindex)]
        M = self.quad.moment(d); Q = self.quad.shear(d); S = self.quad.membrane(d); t = self.quad.t
...
class ModeloFEM:
    ...
    def add_carga_nodal(self, caso, nodo, F): self.cargas_nodales.append({"caso": caso, "nodo": nodo, "F": list(F)})
    ...
    def _casos(self):
        ...
        for el in self.elementos:
            for c in getattr(el, "cargas", []): cs.add(c["caso"])
        for c in self.cargas_nodales: cs.add(c["caso"])
        return sorted(cs)

    def resolver(self, combos=None):
        nindex = self._nindex(); ndof = len(self.norden)*GDL; casos = self._casos()
        if combos is None: combos = {c: {c: 1.0} for c in casos}
        # ensamblaje disperso por tripletes (COO) -- vectorizado por elemento
        rows = []; cols = []; data = []
        for el in self.elementos:
            dofs = np.asarray(el.dofs(nindex)); Kg = np.asarray(el.Kglobal)
            n = len(dofs)
            rr = np.repeat(dofs, n); cc = np.tile(dofs, n)
            rows.append(rr); cols.append(cc); data.append(Kg.flatten())
        ...
        for el in self.elementos:
            ...
            for c in casos:
                ...
                for a, ia in enumerate(dofs): Fc[c][ia] -= fg[a]
        for cn in self.cargas_nodales:
            base = nindex[cn["nodo"]]*GDL
            for i in range(6): Fc[cn["caso"]][base+i] += cn["F"][i]
        ...
        for cname, factors in combos.items():
            ...
            for el in self.elementos:
                if isinstance(el, ElementoBarra):
                    acc = np.zeros(12)
                    for c, f in factors.items():
                        if c in Uc: acc += f*el.esfuerzos(Uc[c], nindex, c)
                    # esfuerzo interno (convencion PyNite): extremo i = f[0:6];
                    # extremo j = -f[6:12]. axial(0)=N_i, etc.
                    esf_b[el.eid] = {
            ...
            res["combos"][cname] = {"desplazamientos": disp, "reacciones": reac,
                                    "esfuerzos_barra": esf_b, "esfuerzos_lamina": esf_l, "equilibrio": eq}
        return res
