...
import os, sys; sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))  # gen_ifc local
...
def cajon(entrada,out):
    """Cajon postesado unicelular trapezoidal: IfcBeam con perfil
    IfcArbitraryProfileDefWithVoids (geometria REAL) + Psets (dims de pared,
    postesado, cargas). Eje X; perfil en plano (horizontal,vertical)=(ancho,canto)."""
    c=entrada["cajon"]; po=entrada["postesado"]; mat=dict(c["material"]); mat["_nombre"]="HP-cajon"
    bs=c["bs"]; bi=c["bi"]; h=c["h"]; tt=c["t_top"]; tb=c["t_bot"]; tw=c["t_web"]
    nv=int(c.get("n_vanos",1)); Ltot=c["L"]*nv
    m,body,site=G.nuevo(entrada.get("nombre","CAJON"))
    # perfil: (u=ancho horizontal, v=canto vertical). top en v=+h/2 ancho bs; bot en v=-h/2 ancho bi
    outer=[(-bi/2,-h/2),(bi/2,-h/2),(bs/2,+h/2),(-bs/2,+h/2)]
    inner=[(-(bi/2-tw),-h/2+tb),((bi/2-tw),-h/2+tb),((bs/2-tw),h/2-tt),(-(bs/2-tw),h/2-tt)]
    prof=G._profile_box(m,outer,inner)
    b=G.elem(m,body,site,"IfcBeam","CAJON",prof,Ltot,base=(0,0,0),dirv=(1,0,0))
    G.material(m,b,mat)
    cj={"Rol":"viga","L":c["L"],"n_vanos":nv,"bs":bs,"bi":bi,"h":h,
        "t_top":tt,"t_bot":tb,"t_web":tw,
    ...
    cj={k:v for k,v in cj.items() if v is not None}
    G.pset(m,b,"Pset_Estructurando_Cajon",cj)
    G.pset(m,b,"Pset_Estructurando_Postesado",{k:v for k,v in po.items() if v is not None})
    m.write(out); print("IFC cajon:",out)
_B["cajon"]=cajon

def mixto(entrada,out):
    """Tablero MIXTO: N vigas de acero (IfcBeam, IfcIShapeProfileDef = geometria
    REAL) + losa de hormigon (IfcSlab). Pset_Estructurando_Tipologia=mixto +
    Pset_Estructurando_Mixto (dims de perfil, conector, fatiga). PT 7.5."""
    mx=entrada["mixto"]; pf=mx["perfil"]
    ms=mx["material_s"]; mh=mx["material_h"]
    mat_s=dict(ms); mat_s["_nombre"]="S-acero"
    mat_h=dict(mh); mat_h["_nombre"]="HA-losa"
    L=mx["L"]; B=mx["B"]; ng=int(mx["n_vigas"]); t=mx["t_losa"]
    m,body,site=G.nuevo(entrada.get("nombre","MIXTO"))
    import numpy as _np
    ysg=list(_np.linspace(-B/2+B/(2*ng),B/2-B/(2*ng),ng))
    prof=G._ishape(m,pf["b"],pf["h"],pf["tw"],pf["tf"])
    for i,yg in enumerate(ysg):
        b=G.elem(m,body,site,"IfcBeam","VIGA_%d"%i,prof,L,base=(0,yg,-(t/2+pf["h"]/2)),dirv=(1,0,0))
        G.material(m,b,mat_s)
    slab=G.elem(m,body,site,"IfcSlab","LOSA",G._rect(m,L,B),t,base=(0,0,0),predef="FLOOR")
    G.material(m,slab,mat_h)
    G.pset(m,slab,"Pset_Estructurando_Tipologia",{"Tipologia":"mixto"})
    co=mx.get("conector",{}); fa=mx.get("fatiga",{})
    G.pset(m,slab,"Pset_Estructurando_Mixto",{
        "L":L,"n_vanos":int(mx.get("n_vanos",1)),"B":B,"n_vigas":ng,"t_losa":t,
        "nx":int(mx.get("nx",16)),"ny":int(mx.get("ny",10)),
        "n_carriles":int(mx.get("n_carriles",2)),"g2_N_m2":mx.get("g2_N_m2",0),
        "perfil_h":pf["h"],"perfil_b":pf["b"],"perfil_tw":pf["tw"],"perfil_tf":pf["tf"],
        "fu_acero":ms.get("fu",490e6),
        "conector_d":co.get("d",0.022),"conector_hsc":co.get("hsc",0.150),
        "conector_fu":co.get("fu",450e6),"conector_sep":co.get("sep_long",0.20),
        "conector_nfila":int(co.get("n_fila",2)),
        "fat_detalle_MPa":fa.get("detalle_MPa",80.0),"fat_lambda":fa.get("lambda",1.0),
        "fat_phi":fa.get("phi",1.0)})
    m.write(out); print("IFC mixto:",out)
_B["mixto"]=mixto


def oblicuo(entrada,out):
    """Tablero OBLICUO: losa (IfcSlab) + Pset_Estructurando_Tipologia=oblicuo +
    Pset_Estructurando_Oblicuo (esviaje, dims, armado). PT 7.5."""
    ob=entrada["oblicuo"]; mat=dict(ob["material"]); mat["_nombre"]="HA-losa-obl"
    L=ob["L"]; B=ob["B"]; t=ob["t"]
    m,body,site=G.nuevo(entrada.get("nombre","OBLICUO"))
    slab=G.elem(m,body,site,"IfcSlab","LOSA_OBL",G._rect(m,L,B),t,base=(0,0,0),predef="FLOOR")
    G.material(m,slab,mat)
    G.pset(m,slab,"Pset_Estructurando_Tipologia",{"Tipologia":"oblicuo"})
    G.pset(m,slab,"Pset_Estructurando_Oblicuo",{
        "L":L,"B":B,"esviaje_deg":ob.get("esviaje_deg",0.0),"t":t,
        "nx":int(ob.get("nx",16)),"ny":int(ob.get("ny",12)),
        "n_carriles":int(ob.get("n_carriles",2)),"g2_N_m2":ob.get("g2_N_m2",0),
        "recubrimiento":ob.get("recubrimiento",0.035),"fyk":ob["material"].get("fyk",500e6)})
    m.write(out); print("IFC oblicuo:",out)
_B["oblicuo"]=oblicuo


def curvo(entrada,out):
    """Tablero CURVO: viga-cajon (IfcBeam, IfcArbitraryProfileDefWithVoids =
    geometria REAL) + Pset_Estructurando_Tipologia=curvo + Pset_Estructurando_Curvo
    (radio R, dims). El eje puede venir de un IfcAlignment (Ola 5). PT 7.5."""
    c=entrada["curvo"]; mat=dict(c["material"]); mat["_nombre"]="HA-curvo"
    bs=c["bs"]; bi=c["bi"]; h=c["h"]; tt=c["t_top"]; tb=c["t_bot"]; tw=c["t_web"]
    S=c["L"]
    m,body,site=G.nuevo(entrada.get("nombre","CURVO"))
    outer=[(-bi/2,-h/2),(bi/2,-h/2),(bs/2,+h/2),(-bs/2,+h/2)]
    inner=[(-(bi/2-tw),-h/2+tb),((bi/2-tw),-h/2+tb),((bs/2-tw),h/2-tt),(-(bs/2-tw),h/2-tt)]
    prof=G._profile_box(m,outer,inner)
    b=G.elem(m,body,site,"IfcBeam","CURVO",prof,S,base=(0,0,0),dirv=(1,0,0))
    G.material(m,b,mat)
    G.pset(m,b,"Pset_Estructurando_Tipologia",{"Tipologia":"curvo"})
    G.pset(m,b,"Pset_Estructurando_Curvo",{
        "R":c["R"],"L":S,"bs":bs,"bi":bi,"h":h,"t_top":tt,"t_bot":tb,"t_web":tw,
        "nx":int(c.get("nx",16)),"nyt":int(c.get("nyt",6)),"nyb":int(c.get("nyb",3)),
        "nz":int(c.get("nz",3)),"g2_N_m2":c.get("g2_N_m2",0)})
    m.write(out); print("IFC curvo:",out)
_B["curvo"]=curvo
